...
def apply_content_changes(course, changes, full_gmo_text, api_key, progress_callback=None):
    """Apply LLM-powered changes. Sends full GMO as context so LLM can follow cross-references."""
    import anthropic
    client = anthropic.Anthropic(api_key=api_key)
    log = []
    content_changes = [c for c in changes if c['action'] in LLM_ACTIONS]

    # Truncate GMO to fit context (keep first 80K chars — covers Parts 1 & 2)
    gmo_context = full_gmo_text[:80000]

    for i, change in enumerate(content_changes):
        if progress_callback:
            progress_callback(f"Processing ({i+1}/{len(content_changes)}): {change['title'][:60]}")
...
def run_dede(imscc_bytes, gmo_text='', style='none', api_key=None, progress_callback=None):
    ...
    # Step 5: Content changes (LLM)
    content_changes = [c for c in changes if c['action'] in LLM_ACTIONS]
    if content_changes and api_key:
        content_log = apply_content_changes(course, changes, full_gmo, api_key, progress_callback)
        log.extend(content_log)
    elif content_changes and not api_key:
        log.append(f"⚠️ {len(content_changes)} content changes require an API key — skipped")
